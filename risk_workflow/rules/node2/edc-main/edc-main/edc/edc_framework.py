...
except Exception:  # pragma: no cover - optional in OpenAI-only path
    MistralForSequenceEmbedding = None
try:
...
class EDC:
    ...
    @staticmethod
    def _print_stage_output(stage: str, index: int, payload) -> None:
        try:
            content = json.dumps(payload, ensure_ascii=False)
        except Exception:
            content = str(payload)
        print(f"[EDC][{stage}][index={index}] {content}", flush=True)
    ...
    def oie(
        ...
        if previous_extracted_triplets_list is not None:
            ...
            oie_refinement_prompt_template_str = self._read_text_file(self.oie_r_prompt_template_file_path)
            oie_refinement_few_shot_examples_str = self._read_text_file(self.oie_r_few_shot_example_file_path)
            ...
            for idx, triples in enumerate(oie_triples_list):
                self._print_stage_output("OIE_REFINED", idx, triples)
        else:
            # Normal OIE
            entity_hint_list = ["" for _ in input_text_list]
            relation_hint_list = ["" for _ in input_text_list]
            logger.info("Running OIE...")
            oie_few_shot_examples_str = self._read_text_file(self.oie_few_shot_example_file_path)
            oie_few_shot_prompt_template_str = self._read_text_file(self.oie_prompt_template_file_path)
            ...
            for idx, triples in enumerate(oie_triples_list):
                self._print_stage_output("OIE", idx, triples)
            ...
            if model_type == "hf":
                if AutoModelForCausalLM is None or AutoTokenizer is None:
                    raise RuntimeError(
                        ...
                        "Use OPENAI-compatible model names to avoid local HF loading."
                    )
                model, tokenizer = (
                ...
                if llm_utils.is_openai_embedding_model(model_name):
                    model = llm_utils.OpenAIEmbeddingEncoder(model_name)
                else:
    ...
    def schema_definition(self, input_text_list: List[str], oie_triplets_list: List[List[str]], free_model=False):
        ...
        schema_definition_few_shot_prompt_template_str = self._read_text_file(self.sd_template_file_path)
        schema_definition_few_shot_examples_str = self._read_text_file(self.sd_few_shot_example_file_path)
        ...
        for idx, oie_triplets in enumerate(tqdm(oie_triplets_list)):
            ...
            self._print_stage_output("SCHEMA_DEFINITION", idx, schema_definition_dict)
    ...
    def schema_canonicalization(
        ...
        sc_verify_prompt_template_str = self._read_text_file(self.sc_template_file_path)
        ...
        for idx, input_text in enumerate(tqdm(input_text_list)):
            ...
            self._print_stage_output(
                "SCHEMA_CANONICALIZATION",
                idx,
                {
                    "canonicalized_triplets": canonicalized_triplets,
                    "candidates": canon_candidate_dict_list,
                },
            )
    ...
    def construct_refinement_hint(
        ...
        entity_extraction_few_shot_examples_str = self._read_text_file(self.ee_few_shot_example_file_path)
        entity_extraction_prompt_template_str = self._read_text_file(self.ee_template_file_path)

        entity_merging_prompt_template_str = self._read_text_file(self.em_template_file_path)
    ...
    def extract_kg(self, input_text_list: List[str], output_dir: str = None, refinement_iterations=0):
        ...
        for iteration in range(refinement_iterations + 1):
            ...
            for idx, canon_triplets in enumerate(non_null_triplets_list):
                ...
                self._print_stage_output("FINAL_KG", idx, canon_triplets)
