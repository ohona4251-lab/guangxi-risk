...
try:
    ...
    GenerationConfig = Any  # type: ignore
import ast
try:
...
def is_model_openai(model_name):
    ...
    if "gpt" in name or name.startswith("glm-"):
        return True
    # Use OpenAI-compatible endpoint for simple model ids when base URL is configured.
...
def is_openai_embedding_model(model_name: str) -> bool:
    ...
    if name.startswith("embedding-"):
        return True
    # Allow OpenAI-compatible embedding model ids with custom base URL.
    if os.getenv("OPENAI_BASE_URL") and "/" not in model_name:
        return True
    return False
...
def _build_openai_client() -> OpenAI:
    api_key = os.getenv("OPENAI_API_KEY") or os.getenv("OPENAI_KEY") or os.getenv("DASHSCOPE_API_KEY")
    if not api_key:
        raise RuntimeError("Missing OPENAI_API_KEY / OPENAI_KEY / DASHSCOPE_API_KEY")
...
class OpenAIEmbeddingEncoder:
    """SentenceTransformer-compatible wrapper backed by OpenAI embeddings API."""
    ...
    def encode(self, text: str, prompt_name=None, prompt=None):
        if prompt:
            text = f"{prompt}{text}"
        res = self.client.embeddings.create(model=self.model_name, input=text)
